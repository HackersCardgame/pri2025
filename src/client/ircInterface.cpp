...
#ifndef TCP_CLIENT_IRC
#define TCP_CLIENT_IRC
...
class TcpClientNet
{
  private:
    ...
    inline static string user = "ircuser";
    inline static string password = "123456";                                   //TO have the user in DB run: user@debian:~/pri42/tools/install$ python3 dbConfigFiles.py --import ircUser.cfg
    inline static string command = "";                                          //if irc line becins with ! like !site zh then the programm must be called from the irc bot
  ...
  public:
    ...
    TcpClientNet(string host, string user, string password, string ircUser, string command, int retries)
    {
      if ( TcpClientNet::debug ) cout << "TcpClientNet()" << endl;

      TcpClientNet::TCPHOST=host;
      TcpClientNet::user=user+"?"+ircUser;
      TcpClientNet::password=sha256(password);
      TcpClientNet::command=command;
    ...
    static void serverListener(int sockfd)
    {
      ...
      SSL_write(gSsl, TcpClientNet::command.c_str(), TcpClientNet::command.length());
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        string content = string(buff);
//        string colorStriped = RegexFunctions::replace("\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[mGK]", "x", content);
        string colorStriped = RegexFunctions::replace("\\x1b\[[0-9;]*[a-zA-Z]", "", content);  //XXX: REMOVE THE COLOR INFO for IRC
        ...
        for ( auto l : lines )
        {
          ...
          if ( lineNumber > 10 )
          {
            cout << "==> more than 10 Lines, TRUNCATED, use phaseClient or be more specific <==" << endl;
            break;
          }
          if ( !RegexFunctions::match("successfully logged in on", l ) 
            && !RegexFunctions::match("\n<EOF>", l) 
            && !RegexFunctions::match("available commands:", l)
            && l != "" )
            cout << l << endl;
        }
        //cout << colorStriped << endl;
        answered = true;
...
int main(int argc, char* argv[])
{
  ...
  if (argc > 2 )
    ircUser = string(argv[2]);
    
  TcpClientNet tcn = TcpClientNet(TcpClientNet::TCPHOST, "ircuser", "123456", ircUser, command, 0);
