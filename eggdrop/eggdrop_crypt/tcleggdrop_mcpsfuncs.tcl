...
#  02/07/04 - add abilility to override any encryption on output by prefixing output string with NOCRYPT
...
# to override any encryption on output of any string, prefix output string with NOCRYPT
#######################################################
...
source eggdrop_crypt/tcleggdrop_mcps_sitesettings.ini
#######################################################
...
proc stdrag_trybind { comtype nick uhost hand chan text table flags mask hits callback} { 
  ...
  if {$table == "pub"} {
      ...
      catch { eval $callback [list $nick $uhost $hand $chan $remaindertext] }
    } elseif {$table == "msg"} {
      # these commands dont want the initial trigger mask(command)
      set startpos [string length $mask]
      incr startpos
      set remaindertext [string range $text $startpos end]
      catch { eval $callback [list $nick $uhost $hand $remaindertext] }
    } elseif {$table == "pubm"} {
      catch { eval $callback [list $nick $uhost $hand $chan $text] }
    } elseif {$table == "msgm"} {
      catch { eval $callback [list $nick $uhost $hand $text] }
    } elseif {$table =="notc"} {
      catch { eval $callback [list $nick $uhost $hand $text $chan] }
    } elseif {$table =="ctcp"} {
      #ATTN: UNTESTED
      global dcckeyword
      catch { eval $callback [list $nick $uhost $hand $chan $dcckeyword $text] }
    } elseif {$table =="chat"} {
      #ATTN: UNTESTED
      catch { eval $callback [list $hand $chan $text] }
    } elseif {$table =="dcc"} {
      #ATTN: UNTESTED
      #catch { eval $callback [list $hand $dccidx $text] }
    } else {
      catch { eval $callback [list $nick $uhost $hand $chan $text] }
    }
...
#######################################################
proc encryptout {callcommand prefixkeyword dest inmsg} {
  ...
  while {$inmsg!=""} {
    ...
    if { $newmsg != "" } {
      ...
      #now invoke it
      #putlog "DEBUG in encryptout command='$callcommand'  prefix='$prefixkeyword'  dest='$dest'  inmsg='$inmsg'  newmsg='$newmsg'  newtext='$newtext'"
      catch { eval $callcommand [list $newtext] }
      }
    } 
  }
#######################################################
...
proc putservmc {putlinetext} {
  ...
  # NEW REPLACEMENT
  encryptout trueputserv $prefixkeyword $dest $msg
  # encrypt it
  set newmsg [ mcpsencrypt $dest $msg ]
  #now send the rebuilt string
  if { $newmsg != "" } {
...
proc puthelpmc {putlinetext} {
  # find the ':' which separates stuff like "NOTICE CHAN: HERE IS MY TEXT"
  set searchc ":"
  set colonpos [ string first $searchc $putlinetext ]
  if {$colonpos == -1 || $colonpos == 0} {
    # some raw commands to irc dont have : and they should be sent unencrypted
    trueputhelp $putlinetext
    return
    }
  # grab the initial 'PRIVMSG CHAN :'
  set partone [ string range $putlinetext 0 [expr $colonpos - 1 ] ]
...
proc putquickmc {putlinetext} {
  ...
  set msg [ string range $putlinetext [ expr $colonpos + 1 ] end ]
  # NEW REPLACEMENT
  encryptout trueputquick $prefixkeyword $dest $msg
  # encrypt it
  set newmsg [ mcpsencrypt $dest $msg ]
  #now send the rebuilt string
  if { $newmsg != "" } {
...
proc putnowmc {putlinetext} {
  ...
  set msg [ string range $putlinetext [ expr $colonpos + 1 ] end ]
  # NEW REPLACEMENT
  encryptout trueputnow $prefixkeyword $dest $msg
  # encrypt it
  set newmsg [ mcpsencrypt $dest $msg ]
  #now send the rebuilt string
  if { $newmsg != "" } {
...
proc putmsgmc {dest msg} {
  # send mesg
  set prefixkeyword "PRIVMESG"
  # NEW REPLACEMENT
  encryptout trueputmsg $prefixkeyword $dest $msg
  # encrypt it
  set newmsg [ mcpsencrypt $dest $msg ]
  #now send the rebuilt string
  if { $newmsg != "" } {
...
proc putnotcmc {dest msg} {
  ...
  # NEW REPLACEMENT
  encryptout trueputnotc $prefixkeyword $dest $msg
  # encrypt it
  set newmsg [ mcpsencrypt $dest $msg ]
  #now send the rebuilt string
  if { $newmsg != "" } {
...
proc trueputserv {putlinetext} {
  global mcautomagic
  if {$mcautomagic != "true"} { putserv $putlinetext } else { _mcorig_putserv $putlinetext }
  }

proc trueputhelp {putlinetext} {
  global mcautomagic
  if {$mcautomagic != "true"} { puthelp $putlinetext } else { _mcorig_puthelp $putlinetext }
  }

proc trueputquick {putlinetext} {
  global mcautomagic
  if {$mcautomagic != "true"} { putquick $putlinetext } else { _mcorig_putquick $putlinetext }
  }

proc trueputnow {putlinetext} {
  global mcautomagic
  if {$mcautomagic != "true"} { putnow $putlinetext } else { _mcorig_putnow $putlinetext }
  }

proc trueputmsg {dest msg} {
  global mcautomagic
  if {$mcautomagic != "true"} { putmsg $putlinetext } else { _mcorig_putmsg $putlinetext }
  }

proc trueputnotc {dest msg} {
  global mcautomagic
  if {$mcautomagic != "true"} { putnotc $putlinetext } else { _mcorig_putnotc $putlinetext }
  }
#######################################################
...
proc mcpsdecrypt {c t} {
 ...
 if {$firstword == "mcps" || $firstword == "+OK"} {
  ...
  if {$chankey != ""} {set t [decrypt $chankey $t] }
  }
 return $t
}
...
proc dcc_mcbinds {h i t} {
    ...
    foreach bind $::mcbinds {
        regexp -- $mcbinds_pattern $bind - bt bf bm bh bc
